...
pub fn decode_jpeg(data: &[u8]) -> DcmResult<JpegFrame> {
    ...
    let raw = dec
        .decode()
        .map_err(|e| DcmError::DecompressionError { reason: format!("JPEG decode error: {e}") })?;
    let info: ImageInfo = dec
        .info()
        .ok_or_else(|| DcmError::DecompressionError { reason: "JPEG decoder returned no image info".to_string() })?;
...
mod tests {
    ...
    const TINY_JPEG: &[u8] = &[
        ...
        0x08, 0x09, 0x0A, 0x0B,
        0xFF, 0xC4, 0x00, 0xB5, 0x10, // DHT (AC) — standard baseline AC
        ...
        0x3F, 0x00,
        0xF8, // compressed data (single all-zero 8x8 block → grey pixel)
        0xFF, 0xD9, // EOI
    ];
